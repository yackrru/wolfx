...
func TestCustomMapperTypeWrite(t *testing.T) {
	db, err := sql.Open("sqlite3", ":memory:")
	if err != nil {
		t.Fatal(err)
	}
	...
	for rows.Next() {
		...
		assert.Equal(t, chunk[count].Props.(TestChunkType).Id, id)
		assert.Equal(t, chunk[count].Props.(TestChunkType).Name, name)
		count++
	}
}
